    ...
    <div className="grid grid-cols-3 gap-3">
      {data.fundamentalMetrics.map((m, i) => (
        <div key={i} className="bg-accent-area border border-border p-3.5">
          ...
          <p className="text-[12px] text-muted-foreground mt-1">{m.note}</p>
        </div>
      ))}
    </div>
    <div className="mt-4 p-3.5 bg-accent-area border border-border text-[13px] text-muted-foreground leading-[1.7]">
