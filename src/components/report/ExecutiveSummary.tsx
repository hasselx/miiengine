  ...
  <SectionWrapper num="01" title="Executive Summary">
    <div className="space-y-4">
      {data.executiveSummary.map((p, i) => (
        <p key={i} className="text-[14px] leading-[1.8] text-foreground" dangerouslySetInnerHTML={{ __html: p }} />
      ))}
    </div>

    {/* 10-Model Research Abstract */}
    <div className="mt-6 border border-border">
      <div className="px-4 py-2.5 bg-accent-area border-b border-border">
        <p className="font-mono text-[11px] tracking-[2px] uppercase text-muted-foreground font-semibold">10-Model Research Abstract</p>
      </div>
          ...
          <div key={i} className="flex items-start gap-3 px-4 py-2.5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-mono text-[12px] font-semibold text-foreground">{m.model}</span>
                <span className="font-mono text-[10px] text-muted-foreground">— {m.firm}</span>
                <span className={`w-2 h-2 rounded-full shrink-0 ${sentDot[m.sentiment]}`} />
              </div>
              <p className="text-[12px] text-muted-foreground mt-0.5 leading-relaxed">{m.abstract}</p>
            </div>
          </div>
        ))}
      </div>
